...
eval $CMD_PREFIX "/apps/soca/$SOCA_CONFIGURATION/python/latest/bin/python3 /apps/soca/$SOCA_CONFIGURATION/cluster_manager/add_nodes.py \
    --ht_support true --instance_ami $AMI_ID --base_os \"centos7\" --root_size 10 --subnet_id $SUBNET_ID \
    --instance_type t3.large --desired_capacity 1 --queue alwayson --job_owner $USER --job_name $USER \
